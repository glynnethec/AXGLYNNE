...
./src/app/page.tsx (2:1)
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      tl.to(seq, { xOffset: -25, scaleMultiplier: 1.0, duration: 60, ease: 'power1.inOut', onUpdate: render }, 0) // Pan left & scale down
        .to(seq, { xOffset: 0, duration: 40, ease: 'power1.inOut', onUpdate: render }, 60); // Return to center

      // Polarized glass fades in
      tl.to(overlayRef.current, { opacity: 1, duration: 20, ease: 'power1.inOut' }, 50);

      // --- STORYTELLING COREOGRAFIA (Minimalista) ---
      // Initial States
      gsap.set([phase2Ref.current, phase3Ref.current], { 
        opacity: 0, 
        y: 40,
        position: 'absolute',
        top: '50%',
      ...
      mm.add("(min-width: 769px)", () => {
        gsap.set(phase1Ref.current, { left: '10%', right: 'auto', xPercent: 0, textAlign: 'left' });
        gsap.set(phase2Ref.current, { left: '50%', right: 'auto', xPercent: -50, textAlign: 'center' });
        gsap.set(phase3Ref.current, { left: '50%', right: 'auto', xPercent: -50, textAlign: 'center' });
      });
    ...
    return () => {
      ...
      <section className="intro-hero">
        {/* Vanta WebGL Interactive Fog Background */}
      lenis.destroy();
    };
  }, []);
  ...
  return (
    <main>
      ...
      <section className="intro-hero" style={{ position: 'relative' }}>
        {/* Vanta WebGL Interactive Fog Background */}
